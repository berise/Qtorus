...
//	Pin과 토러스를 주고 받는 함수
void TorusInternal::push_front_pipe(int aCol, int aTorusColor)
{
	TPipe	*aPipePtr = m_pPV->at( aCol );
...
void TorusInternal::pop_front_pipe( int aCol )
{
	TPipe	*aPipePtr = m_pPV->at( aCol );
	aPipePtr->pop_front();
...
int TorusInternal::front_pipe(int aCol)
{
	TPipe	*aPipePtr = m_pPV->at( aCol );
...
int TorusInternal::back_pipe( int aCol )
{
	TPipe	*aPipePtr = m_pPV->at( aCol );
...
//	다른 색상인가?
struct  not_equal_color_to : std::unary_function< TPipe *, bool >
{
...
struct  equal_color : std::binary_function< TPipe *, TPipe *, bool >
{
	equal_color( int Row ) : theRow(Row) {}

	//	bind2nd를 이용해 결합할 때 bool operator()는 const가 되어야 한다. 
	bool operator()( TPipe * a, TPipe * b ) const
	{ 
...
void TorusInternal::clearPipe( ClearMethod aClearMethod  )
{
	...
	for( int row = MinPipeSize - 1; row >= 0; row-- )
	{
		...
		TPipe * aZerothPipePtr  = m_pPV->at( 0 ) ;
		int ZerothColor = aZerothPipePtr->getTorusColor(row);

		TPipeVector::iterator result = find_if(	
					m_pPV->begin(),
					m_pPV->end(), 
...
void TorusInternal::insertRoller()	//insertRoller( int nHeight )
{
	...
	TRoller *aRollerPtr	= new TRoller;
